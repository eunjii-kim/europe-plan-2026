 ...
 */
import { convertToKrw } from './budgetCalc.js';
 ...
 * @param {Array<{ amount: number, currency: string }>} expenses
 * @param {{ CHF: number, EUR: number }} rates
 * @returns {number}
 */
export function calcExpenseTotalKrw(expenses, rates) {
  return expenses.reduce((sum, item) => sum + convertToKrw(item.amount, item.currency, rates), 0);
}

/**
 * 지출 기록을 분류별로 묶어 KRW 소계를 계산한다.
 * @param {Array<{ category: string, amount: number, currency: string }>} expenses
 * @param {{ CHF: number, EUR: number }} rates
 * @returns {Array<{ category: string, totalKrw: number }>} totalKrw 내림차순 정렬
 */
export function groupExpenseByCategory(expenses, rates) {
  const totals = new Map();
  for (const item of expenses) {
    const prev = totals.get(item.category) || 0;
    totals.set(item.category, prev + convertToKrw(item.amount, item.currency, rates));
  }
 ...
 * @param {Array<{ region: string, amount: number, currency: string }>} expenses
 * @param {{ CHF: number, EUR: number }} rates
 * @returns {Array<{ region: string, totalKrw: number }>} totalKrw 내림차순 정렬
 */
export function groupExpenseByRegion(expenses, rates) {
  const totals = new Map();
  for (const item of expenses) {
    const region = item.region || UNSPECIFIED_REGION_LABEL;
    const prev = totals.get(region) || 0;
    totals.set(region, prev + convertToKrw(item.amount, item.currency, rates));
  }
  return [...totals.entries()]
 ...
 */
export function groupExpenseByCountry(expenses, rates) {
  const totals = new Map();
  for (const item of expenses) {
    const country = resolveCountry(item.region);
    const prev = totals.get(country) || 0;
    totals.set(country, prev + convertToKrw(item.amount, item.currency, rates));
  }
  return [...totals.entries()]
