...
index.html
  ...
  └─ js/app.js             (엔트리포인트, <script type="module">로 로드)
       ...
       │    └─ js/firebaseConfig.js
       ├─ js/customRegions.js   (Firestore CRUD - 사용자가 추가한 지역/도시)
       │    └─ js/firebaseConfig.js
...
- **`js/constants.js`**: 여행 정보, 폴백 환율, Firestore 문서 ID, 카테고리별 아이콘, 정보 탭 장소 분류(`PLACE_CATEGORIES`)와 소비기록 분류(`EXPENSE_CATEGORIES`), 스위스 지역 목록(`SWISS_REGIONS`), 지역 select에서 제외할 지역(`EXCLUDED_REGIONS`)과 "+ 새 지역 추가" sentinel 값(`ADD_NEW_REGION_VALUE`), 테마/편집모드 저장 키 등 프로젝트 전역 상수. 다른 모든 모듈이 이 파일을 참조한다.
