...
index.html
  ...
  └─ js/app.js             (엔트리포인트, <script type="module">로 로드)
       ...
       │    └─ js/firebaseConfig.js
       ├─ js/storage.js         (Firebase Storage 업로드 - 첨부 파일)
       │    ├─ js/firebaseConfig.js
       │    └─ js/constants.js
...
- **`js/firebaseConfig.js`**: Firebase 프로젝트(`europe-plan-2026`) 연결 설정과 Firestore 컬렉션 참조(`budgetItemsCollection`, `budgetOverridesCollection`, `scheduleOverridesCollection`, `scheduleCustomBlocksCollection`), Storage 인스턴스(`storage`)를 내보낸다.
...
- **`js/schedule.js`**: `budget.js`와 동일한 패턴으로 일정 편집(`scheduleOverrides`)과 사용자가 추가한 일정(`scheduleCustomBlocks`)의 실시간 구독/추가·수정·삭제 함수를 제공한다. 첨부(attachments) 배열도 이 두 컬렉션의 문서 필드로 함께 저장된다.
- **`js/storage.js`**: 편집모드에서 첨부 이미지를 파일로 업로드할 때 Firebase Storage에 올리고 다운로드 URL을 반환한다. Storage는 Firestore와 별개로 Firebase 콘솔에서 활성화해야 동작한다(README.md 참고).
...
- **스크롤 스파이 rootMargin**: `scrollSpy.js`는 `.tab-bar` + `.day-nav`의 실제 렌더링 높이를 더해 `rootMargin` 상단을 계산하고, 하단도 좁혀서 "스티키 헤더 바로 아래에 걸린 카드"만 활성으로 잡는다. `renderDayList`가 매번 `innerHTML = ''`로 카드를 새로 만들기 때문에, 재렌더링 후에는 반드시 `setupScrollSpy`를 다시 호출해 옵저버를 새 DOM 요소에 연결해야 한다.
...
- **로깅 트레이드오프**: 서버가 없는 정적 프론트엔드라 별도 로깅 라이브러리는 과한 선택이라 판단했다. Firestore/Storage/환율 API 호출 실패 시 `console.error`로 최소한의 디버그 정보만 남기고, 사용자에게는 화면 배너로 안내한다.
